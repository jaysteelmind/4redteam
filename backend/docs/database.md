...
The database follows 4RedTeam's hierarchical data model for penetration testing workflows:
...
```bash
docker run --rm -v "$(pwd):/src" --network redteam-network \
  -e DATABASE_URL='postgres://postgres:postgres@pgvector:5432/redteamdb?sslmode=disable' \
  -w /src sqlc/sqlc generate -f sqlc/sqlc.yml
```

This command:
1. Mounts the current directory into the container
2. Connects to the 4RedTeam database network
...
The database package integrates with 4RedTeam's GraphQL API through the converter package:
...
**Log Output Example**:
```
INFO[0000] SELECT * FROM flows WHERE user_id = '1' AND deleted_at IS NULL  component=redteam-gorm duration=2.5ms rows_returned=3
```
...
The database package provides a robust, secure, and performant foundation for 4RedTeam's data layer. By leveraging sqlc for code generation, implementing consistent security patterns, and maintaining comprehensive audit trails, it ensures reliable operation of the autonomous penetration testing system.
...
For developers working with this package, follow the established patterns for security, performance, and maintainability to ensure smooth integration with the broader 4RedTeam ecosystem.

This documentation provides a comprehensive overview of the database package's architecture, functionality, and integration within the 4RedTeam system.
