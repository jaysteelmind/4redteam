...
The `checker` package is responsible for gathering system facts and verifying installation prerequisites for 4RedTeam. It performs comprehensive system analysis to determine the current state of the installation and what operations are available.
...
- Installation status for each component (4RedTeam, Langfuse, Observability)
...
type CheckHandler interface {
    ...
    Gather4RedTeamInfo(ctx context.Context, c *CheckResult) error
...
- **Script Installation**: Verifies 4RedTeam CLI script in /usr/local/bin
...
- Checks updates for: Installer, 4RedTeam, Langfuse, Observability, Worker images
