...
- DOCKER_CERT_PATH migrated to REDTEAM_DOCKER_CERT_PATH (host path) + DOCKER_CERT_PATH set to /opt/4redteam/docker/ssl (container path)
...
export DOCKER_CERT_PATH=/path/to/certs  # auto-migrated to REDTEAM_DOCKER_CERT_PATH on startup
```
