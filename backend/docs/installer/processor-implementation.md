...
- **State machine logic**: `ApplyChanges` implements three-phase stack management (Observability → Langfuse → 4RedTeam) with integrity validation; the wizard performs a pre-phase interactive integrity check with Y/N decision for force mode
...
- **Three-phase execution**: Observability → Langfuse → 4RedTeam with state validation after each phase
