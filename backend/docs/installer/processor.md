...
graph TB
    ...
    subgraph "External Systems"
        ...
        UpdateServer[Update Server<br/>example.com]
...
- **ProductStack**: Logical grouping of services that can be managed as a unit (4redteam, langfuse, observability, worker, installer, all)
...
- **Worker Images**: Container images for AI agent tasks (default: debian:latest, pentest: jaysteelmind/kali-redteam)
...
const (
    Stack4RedTeam        ProductStack = "4redteam"        // Main stack (docker-compose.yml)
...
- **Start(stack)**:
  - 4redteam/langfuse/observability: `docker compose ... up -d` (honors embedded mode for non-destructive ops)
  - all: sequential start in order observability → langfuse → 4redteam
  - worker/installer: not applicable

- **Stop(stack)**:
  - 4redteam/langfuse/observability: `docker compose ... stop`
  - all: sequential stop in reverse order (4redteam → langfuse → observability)
  - worker/installer: not applicable
...
- **Download(stack)**:
  - 4redteam/langfuse/observability: `docker compose pull`
...
- **Install(stack)**:
  - 4redteam: extract compose file and example provider config
...
- **Update(stack)**:
  - 4redteam/langfuse/observability: download → `docker compose up -d`
...
### Removal Operations
- **Remove(stack)**:
  - 4redteam/langfuse/observability: `docker compose down` (keep volumes/images)
  - worker: remove images via Docker API and related containers
  - installer: remove flow stubbed
  - all: remove all stacks

- **Purge(stack)**:
  - 4redteam/langfuse/observability: `down --rmi all -v` for strict purge; standard purge `down -v` is also available
...
- **ApplyChanges()**:
  ...
  - phase 3: 4redteam (always embedded; ensure/verify → update)
...
- **compose.go**:
  - `install4RedTeam()`, `installLangfuse()`, `installObservability()` - extract compose files with environment patching
...
- **docker.go**:
  - `pullWorkerImage()` - download pentest image (DOCKER_DEFAULT_IMAGE_FOR_PENTEST: jaysteelmind/kali-redteam)
...
- **update.go**:
  - `checkUpdates()` - communicate with example.com update server
...
- **Two-Track Command Execution**:
  - Worker stack: Docker API SDK (uses DOCKER_HOST, REDTEAM_DOCKER_CERT_PATH, DOCKER_TLS_VERIFY from config)
...
- **Progress Tracking**: Worker downloads (jaysteelmind/kali-redteam 6GB+ → 13GB disk) with real-time progress via terminal
- **Docker Configuration**: Support NET_ADMIN capability for network scanning, Docker socket access for container management
- **Dependency Ordering**: 4RedTeam must start before Langfuse/Observability for network creation
...
- **Container Naming**: Follows patterns in checker constants (RedTeamContainerName, etc.)

### Key Environment Variables
- **LLM providers**: OPEN_AI_KEY, ANTHROPIC_API_KEY, GEMINI_API_KEY, BEDROCK_*, OLLAMA_SERVER_URL
- **Provider configs**: REDTEAM_LLM_SERVER_CONFIG_PATH (host path), REDTEAM_OLLAMA_SERVER_CONFIG_PATH (host path)
- **Monitoring**: LANGFUSE_BASE_URL, LANGFUSE_PROJECT_ID, OTEL_HOST
- **Docker config**: DOCKER_HOST, REDTEAM_DOCKER_CERT_PATH (host path), DOCKER_TLS_VERIFY, DOCKER_CERT_PATH (container path, managed)
- **Deployment modes**: envs determine embedded vs external vs disabled
- **Worker images**: DOCKER_DEFAULT_IMAGE (debian:latest), DOCKER_DEFAULT_IMAGE_FOR_PENTEST (jaysteelmind/kali-redteam)
- **Path migration**: DoMigrateSettings() migrates old DOCKER_CERT_PATH/LLM_SERVER_CONFIG_PATH/OLLAMA_SERVER_CONFIG_PATH to REDTEAM_* variants on startup
...
- HTTP client for installer updates from example.com
