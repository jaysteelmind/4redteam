...
The processor package manages 4RedTeam stack lifecycle through state-oriented approach. Main idea: maintain consistency between desired user state (state.State) and actual system state (checker.CheckResult).

## Key Principles

1. **State-Driven Operations**: All operations based on comparing current and target state
2. **Stack Independence**: Each stack (observability, langfuse, 4redteam) managed independently
...
└── ProductStack4RedTeam (mandatory, always embedded)
```
...
#### Phase 3: 4RedTeam Stack Management
```go
// 4RedTeam always embedded, always required
p.fsOps.ensureStackIntegrity(ctx, ProductStack4RedTeam, state)
p.composeOps.updateStack(ctx, ProductStack4RedTeam, state)
p.checker.Gather4RedTeamInfo(ctx)
```

**Rationale**: 4RedTeam - main stack, always installed, only file integrity check.
...
- dependency ordering: observability → langfuse → 4redteam
