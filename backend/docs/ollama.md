...
The Ollama provider enables 4RedTeam to use local language models through the [Ollama](https://ollama.ai/) server.
...
Control how 4RedTeam interacts with your Ollama server:
...
**Performance Note:** Enabling `OLLAMA_SERVER_LOAD_MODELS_ENABLED` adds startup latency as 4RedTeam queries the Ollama API. Disable if you only need specific models from config file.
...
The provider supports all 4RedTeam agent types with optimized configurations:
...
# Start 4RedTeam with Ollama provider
./4redteam
```
