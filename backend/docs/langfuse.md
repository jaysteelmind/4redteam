# Langfuse Integration for 4RedTeam

This document provides a comprehensive guide to the Langfuse integration in 4RedTeam, covering architecture, setup, usage patterns, and best practices for developers.

## Table of Contents

- [Langfuse Integration for 4RedTeam](#langfuse-integration-for-4redteam)
...
Langfuse is an open-source observability platform specifically designed for LLM-powered applications. The 4RedTeam integration with Langfuse provides:
...
The Langfuse integration in 4RedTeam is built around a layered architecture that provides both high-level abstractions for simple use cases and fine-grained control for complex scenarios.

```mermaid
flowchart TD
    Application[4RedTeam Application] --> Observer[Observer]
...
```mermaid
sequenceDiagram
    participant App as 4RedTeam Application
...
The main integration point in 4RedTeam is the Flow Controller, which handles the lifecycle of AI agent flows:
