# 4RedTeam Observability Stack

## Table of Contents
- [4RedTeam Observability Stack](#4redteam-observability-stack)
...
The 4RedTeam Observability Stack provides comprehensive monitoring, logging, tracing, and metrics collection for the application. It integrates multiple technologies to provide a complete view of the application's behavior, performance, and health:
...
```mermaid
flowchart TD
    App[4RedTeam Application] --> Observer[Observability Interface]
...
```mermaid
sequenceDiagram
    participant App as 4RedTeam App
...
4RedTeam uses several patterns for instrumenting different parts of the application. These patterns demonstrate best practices for integrating the observability stack.

### HTTP Server Instrumentation

The 4RedTeam application uses a Gin middleware to instrument HTTP requests, located in `pkg/server/logger/logger.go`:
...
4RedTeam also provides instrumentation for GraphQL operations:
