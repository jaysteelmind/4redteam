# 4RedTeam Configuration Guide

This document serves as a comprehensive guide to the configuration system in 4RedTeam, primarily aimed at developers. It details all available configuration options, their purposes, default values, and how they're used throughout the application.

## Table of Contents

- [4RedTeam Configuration Guide](#4redteam-configuration-guide)
...
4RedTeam uses environment variables for configuration, with support for `.env` files through the `godotenv` package. The configuration is defined in the `Config` struct in `pkg/config/config.go` and is loaded using the `NewConfig()` function.
...
| -------------- | -------------------- | ---------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| DatabaseURL    | `DATABASE_URL`       | `postgres://redteamuser:redteampass@pgvector:5432/redteamdb?sslmode=disable` | Connection string for the PostgreSQL database with pgvector extension    |
...
| InstallationID | `INSTALLATION_ID`    | *(none)*                                                                     | Unique installation identifier for 4RedTeam Cloud API communication       |
| LicenseKey     | `LICENSE_KEY`        | *(none)*                                                                     | License key for 4RedTeam Cloud API authentication and feature activation  |
...
- **DataDir**: Specifies where 4RedTeam stores persistent data. This is used across multiple components:
...
- **InstallationID**: A unique identifier for the 4RedTeam installation used for cloud API communication:
...
- **LicenseKey**: Authentication key for 4RedTeam Cloud API and premium feature activation:
  - Validates license and enables licensed features
  - Required for enterprise features and support
  - Used for authentication with 4RedTeam Cloud services
...
These settings control how 4RedTeam interacts with Docker, which is used for terminal isolation and executing commands in a controlled environment. They're crucial for the security and functionality of tool execution.

| Option                       | Environment Variable               | Default Value          | Description                                                                                |
| ---------------------------- | ---------------------------------- | ---------------------- | ------------------------------------------------------------------------------------------ |
| DockerInside                 | `DOCKER_INSIDE`                    | `false`                | Set to `true` if 4RedTeam runs inside Docker and needs to access the host Docker daemon.    |
...
| DockerDefaultImageForPentest | `DOCKER_DEFAULT_IMAGE_FOR_PENTEST` | `jaysteelmind/kali-redteam` | Default Docker image for penetration testing tasks                                         |
...
- **DockerInside**: Signals whether 4RedTeam is running inside a Docker container itself, which affects how volumes and sockets are mounted:
  ```go
...
These settings control the HTTP and GraphQL server that forms the backend API of 4RedTeam.
...
These settings control the vector embedding service used for semantic search and similarity matching, which is fundamental for 4RedTeam's intelligent search capabilities.
