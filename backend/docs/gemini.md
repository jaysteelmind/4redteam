...
The Google AI provider enables 4RedTeam to use Google's Gemini language models through the Generative AI API. This provider supports advanced features like function calling, streaming responses, and competitive pricing.
...
docker run --rm \
  -v $(pwd)/.env:/opt/4redteam/.env \
  jaysteelmind/4redteam /opt/4redteam/bin/ctester \
  -config /opt/4redteam/conf/gemini.provider.yml

# Using custom configuration
docker run --rm \
  -v $(pwd)/.env:/opt/4redteam/.env \
  -v $(pwd)/gemini-custom.yml:/opt/4redteam/gemini-custom.yml \
  jaysteelmind/4redteam /opt/4redteam/bin/ctester \
  -type gemini \
  -config /opt/4redteam/gemini-custom.yml
```
...
### Cost Management
- Monitor token usage through 4RedTeam's cost tracking
...
- [4RedTeam Documentation](https://docs.example.com)
- [Issue Tracker](https://github.com/jaysteelmind/4redteam/issues)
