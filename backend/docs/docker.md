...
- [Integration with 4RedTeam](#integration-with-4redteam)
...
The Docker client package (`backend/pkg/docker`) provides a secure and isolated containerized environment for 4RedTeam's AI agents to execute penetration testing operations. This package serves as a wrapper around the official Docker SDK, offering specialized functionality for managing containers that AI agents use to perform security testing tasks.
...
### Role in 4RedTeam Ecosystem

The Docker client is a critical component that enables 4RedTeam's core promise of secure, isolated penetration testing. It provides the foundation for:
...
4RedTeam uses a deterministic port allocation algorithm to ensure each flow gets unique, predictable ports:
...
| `DOCKER_INSIDE` | `false` | Whether 4RedTeam communicates with host Docker daemon from containers |
| `DOCKER_NET_ADMIN` | `false` | Whether 4RedTeam grants the primary container NET_ADMIN capability for advanced networking. |
...
| `DOCKER_DEFAULT_IMAGE_FOR_PENTEST` | `jaysteelmind/kali-redteam` | Default Docker image for penetration testing tasks |
...
type Config struct {
    ...
    DockerDefaultImageForPentest string `env:"DOCKER_DEFAULT_IMAGE_FOR_PENTEST" envDefault:"jaysteelmind/kali-redteam"`
...
The `DOCKER_NET_ADMIN` option controls whether 4RedTeam containers are granted the `NET_ADMIN` Linux capability, which provides advanced networking permissions essential for many penetration testing operations.
...
4RedTeam supports running inside Docker containers while still managing other containers. This is controlled by the `DOCKER_INSIDE` setting:

- **`DOCKER_INSIDE=false`**: 4RedTeam runs on host, manages containers directly
- **`DOCKER_INSIDE=true`**: 4RedTeam runs in container, mounts Docker socket to manage sibling containers

### Network Configuration

When `DOCKER_NETWORK` is specified, all containers are automatically connected to this network, enabling:
- Isolated communication between 4RedTeam components
- Controlled access to external networks
- Service discovery within the 4RedTeam ecosystem

## Core Interfaces

### DockerClient Interface

The main interface defines all Docker operations available to 4RedTeam components:
...
4RedTeam tracks container states in the database:
...
func PrimaryTerminalName(flowID int64) string {
    return fmt.Sprintf("redteam-terminal-%d", flowID)
}
```

This creates names like `redteam-terminal-123` for flow ID 123, making it easy to:
...
4RedTeam implements a multi-layered security approach for container isolation:
...
## Integration with 4RedTeam

### Tool Integration

The Docker client integrates with 4RedTeam's tool system to provide terminal access:
...
- Integrate with 4RedTeam's database for state management
