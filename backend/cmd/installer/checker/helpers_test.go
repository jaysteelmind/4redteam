...
func TestCheckMemoryResources(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := checkMemoryResources(tt.needsForRedteam, tt.needsForGraphiti, tt.needsForLangfuse, tt.needsForObservability)
...
func TestCheckDiskSpaceWithContext(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:              "redteam not installed",
			workerImageExists: true,
			redteamInstalled:  false,
...
func TestCheckUpdatesServer(t *testing.T) {
	...
	t.Run("successful_response", func(t *testing.T) {
		...
		if response.RedteamIsUpToDate {
			t.Error("expected redteam to not be up to date")
		}
	...
	t.Run("with_proxy", func(t *testing.T) {
		...
		proxyTs := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			w.Header().Set("Content-Type", "application/json")
			fmt.Fprintf(w, `{"installer_is_up_to_date": true, "redteam_is_up_to_date": true, "langfuse_is_up_to_date": true, "observability_is_up_to_date": true}`)
		}))
		defer proxyTs.Close()
...
func TestConstants(t *testing.T) {
	...
	if MinFreeMemGBForRedteam <= 0 {
		t.Error("MinFreeMemGBForRedteam should be positive")
	}
	if MinFreeDiskGB <= 0 {
...
func TestCheckUpdatesRequestStructure(t *testing.T) {
	...
	request.RedteamImageName = &imageName
	request.RedteamImageTag = &imageTag
	request.RedteamImageHash = &imageHash
...
func TestCheckVolumesExist(t *testing.T) {
	...
	// 1. Exact match: "redteam-data" matches "redteam-data"
	// 2. Compose prefix match: "redteam-data" matches "redteam_redteam-data"
	// 3. Compose prefix match: "redteam-postgres-data" matches "myproject_redteam-postgres-data"
	//
...
func TestCheckVolumesExist_MatchingLogic(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:            "exact match",
			existingVolumes: []string{"redteam-data", "other-volume"},
			searchVolumes:   []string{"redteam-data"},
			expected:        true,
			description:     "should match exact volume name",
		},
		{
			name:            "compose prefix match",
			existingVolumes: []string{"redteam_redteam-data", "redteam_redteam-ssl"},
			searchVolumes:   []string{"redteam-data"},
			expected:        true,
		...
		},
		{
			...
			existingVolumes: []string{"myproject_redteam-postgres-data", "other_volume"},
			searchVolumes:   []string{"redteam-postgres-data"},
			expected:        true,
		...
		},
		{
			...
			existingVolumes: []string{"redteam-data-backup", "my-redteam-data"},
			searchVolumes:   []string{"redteam-data"},
			expected:        false,
		...
		},
		{
			...
			existingVolumes: []string{"proj_redteam-data", "langfuse-data"},
			searchVolumes:   []string{"redteam-data", "langfuse-data", "missing-volume"},
			expected:        true,
		...
		},
		{
			...
			existingVolumes: []string{"proj1_vol1", "proj2_vol2", "redteam_redteam-ssl"},
			searchVolumes:   []string{"redteam-ssl"},
			expected:        true,
