...
type CheckUpdatesRequest struct {
	...
	RedteamImageName        *string `json:"redteam_image_name,omitempty"`
	RedteamImageTag         *string `json:"redteam_image_tag,omitempty"`
	RedteamImageHash        *string `json:"redteam_image_hash,omitempty"`
...
type CheckUpdatesResponse struct {
	...
	RedteamIsUpToDate       bool `json:"redteam_is_up_to_date"`
...
func checkDirIsWritable(dirPath string) bool {
	...
	tempFile, err := os.CreateTemp(dirPath, ".redteam_test_*")
	if err != nil {
		return false
	}
...
// it matches both exact names and volumes with compose project prefix (e.g., "redteam_redteam-data")
...
func determineComponentNeeds(c *CheckResult) (needsForRedteam, needsForGraphiti, needsForLangfuse, needsForObservability bool) {
	needsForRedteam = !c.RedteamRunning
...
func calculateRequiredMemoryGB(needsForRedteam, needsForGraphiti, needsForLangfuse, needsForObservability bool) float64 {
	requiredGB := MinFreeMemGB
	if needsForRedteam {
		requiredGB += MinFreeMemGBForRedteam
	}
	if needsForGraphiti {
...
func checkMemoryResources(needsForRedteam, needsForGraphiti, needsForLangfuse, needsForObservability bool) bool {
	if !needsForRedteam && !needsForGraphiti && !needsForLangfuse && !needsForObservability {
		return true
	}

	requiredGB := calculateRequiredMemoryGB(needsForRedteam, needsForGraphiti, needsForLangfuse, needsForObservability)
...
func checkDiskSpaceWithContext(
	ctx context.Context,
	workerImageExists, redteamInstalled,
	graphitiConnected, graphitiExternal, graphitiInstalled,
	langfuseConnected, langfuseExternal, langfuseInstalled,
	obsConnected, obsExternal, obsInstalled bool,
) bool {
