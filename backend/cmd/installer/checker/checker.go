...
const (
	...
	RedteamScriptFile            = "/usr/local/bin/4redteam"
	RedteamContainerName         = "4redteam"
	...
	DefaultImageForPentest       = "jaysteelmind/kali-redteam"
	...
	DefaultUpdateServerEndpoint  = ""
	UpdatesCheckEndpoint         = "/api/v1/updates/check"
	UserAgent                    = "4RedTeam-Installer/" + InstallerVersion
	MinFreeMemGB                 = 0.5
	MinFreeMemGBForRedteam       = 0.5
...
type CheckResult struct {
	...
	RedteamScriptInstalled  bool   `json:"redteam_script_installed" yaml:"redteam_script_installed"`
	RedteamExtracted        bool   `json:"redteam_extracted" yaml:"redteam_extracted"`
	RedteamInstalled        bool   `json:"redteam_installed" yaml:"redteam_installed"`
	RedteamRunning          bool   `json:"redteam_running" yaml:"redteam_running"`
	RedteamVolumesExist     bool   `json:"redteam_volumes_exist" yaml:"redteam_volumes_exist"`
	...
	RedteamIsUpToDate       bool   `json:"redteam_is_up_to_date" yaml:"redteam_is_up_to_date"`
...
type CheckHandler interface {
	...
	GatherRedteamInfo(ctx context.Context, c *CheckResult) error
...
func (c *CheckResult) GatherRedteamInfo(ctx context.Context) error {
	if c.handler == nil {
		return ErrHandlerNotInitialized
	}
	return c.handler.GatherRedteamInfo(ctx, c)
}
...
func (c *CheckResult) CanStartAll() bool {
	if c.RedteamInstalled && !c.RedteamRunning {
		return true
	}
...
func (c *CheckResult) CanStopAll() bool {
	return c.RedteamRunning || c.GraphitiRunning || c.LangfuseRunning || c.ObservabilityRunning
}
...
func (c *CheckResult) CanUpdateAll() bool {
	if c.RedteamInstalled && !c.RedteamIsUpToDate {
		return true
	}
...
func (c *CheckResult) CanFactoryReset() bool {
	return c.RedteamInstalled || c.GraphitiInstalled || c.LangfuseInstalled || c.ObservabilityInstalled
}
...
// CanResetPassword returns true when 4RedTeam is running
func (c *CheckResult) CanResetPassword() bool { return c.RedteamRunning }

// CanInstallAll returns true when main stack is not installed yet
func (c *CheckResult) CanInstallAll() bool { return !c.RedteamInstalled }
...
func (h *defaultCheckHandler) GatherAllInfo(ctx context.Context, c *CheckResult) error {
	...
	if err := h.GatherRedteamInfo(ctx, c); err != nil {
		return err
	}
...
func (h *defaultCheckHandler) GatherWorkerInfo(ctx context.Context, c *CheckResult) error {
	...
	dockerCertPath := getEnvVar(h.appState, "REDTEAM_DOCKER_CERT_PATH", "")
...
func (h *defaultCheckHandler) GatherRedteamInfo(ctx context.Context, c *CheckResult) error {
	h.mx.Lock()
	defer h.mx.Unlock()

	envDir := filepath.Dir(h.appState.GetEnvPath())
	dockerComposeFile := filepath.Join(envDir, DockerComposeFile)
	c.RedteamExtracted = checkFileExists(dockerComposeFile) &&
		checkFileExists(ExampleCustomConfigLLMFile) &&
		checkFileExists(ExampleOllamaConfigLLMFile)
	c.RedteamScriptInstalled = checkFileExists(RedteamScriptFile)

	if h.dockerClient != nil {
		exists, running := checkContainerExists(ctx, h.dockerClient, RedteamContainerName)
		c.RedteamInstalled = exists
		c.RedteamRunning = running

		// check if redteam-related volumes exist (indicates previous installation)
		redteamVolumes := []string{"redteam-postgres-data", "redteam-data", "redteam-ssl", "scraper-ssl"}
		c.RedteamVolumesExist = checkVolumesExist(ctx, h.dockerClient, redteamVolumes)
	}
...
func (h *defaultCheckHandler) GatherSystemInfo(ctx context.Context, c *CheckResult) error {
	...
	needsForRedteam, needsForGraphiti, needsForLangfuse, needsForObservability := determineComponentNeeds(c)

	// calculate required memory using shared function
	c.SysMemoryRequired = calculateRequiredMemoryGB(needsForRedteam, needsForGraphiti, needsForLangfuse, needsForObservability)

	// get available memory and check if sufficient
	c.SysMemoryAvailable = getAvailableMemoryGB()
	c.SysMemoryOK = checkMemoryResources(needsForRedteam, needsForGraphiti, needsForLangfuse, needsForObservability)
...
func (h *defaultCheckHandler) GatherUpdatesInfo(ctx context.Context, c *CheckResult) error {
	...
	// get 4RedTeam container image info
	if h.dockerClient != nil && c.RedteamInstalled {
		if imageInfo := getContainerImageInfo(ctx, h.dockerClient, RedteamContainerName); imageInfo != nil {
			request.RedteamImageName = &imageInfo.Name
			request.RedteamImageTag = &imageInfo.Tag
			request.RedteamImageHash = &imageInfo.Hash
		}
	}
	...
	if response != nil {
		...
		c.RedteamIsUpToDate = response.RedteamIsUpToDate
