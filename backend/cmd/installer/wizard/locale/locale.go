...
const (
	...
	WelcomeFormTitle       = "Welcome to 4RedTeam"
	WelcomeFormDescription = "4RedTeam is an autonomous penetration testing platform that leverages AI technologies to perform comprehensive security assessments."
...
The installer guides you through each component setup with recommendations for different deployment scenarios.`
	...
	WelcomeWorkflowStep5 = "5. Deploy and start 4RedTeam services"
...
const (
	...
	TroubleshootEnvFileDesc  = "The .env file is required for 4RedTeam configuration but was not found or is not readable."
...
4. Ensure sufficient disk space is available`
	...
	TroubleshootDockerNotInstalledDesc  = "Docker is not installed on this system. 4RedTeam requires Docker to run containers."
...
5. Try: export DOCKER_HOST=unix:///var/run/docker.sock`
	...
	TroubleshootDockerVersionDesc  = "Your Docker version is incompatible. 4RedTeam requires Docker 20.0.0 or newer."
...
Verify installation: docker compose version`
	...
	TroubleshootComposeVersionDesc  = "Your Docker Compose version is incompatible. 4RedTeam requires Docker Compose 1.25.0 or newer."
...
5. Ensure certificates are valid if using TLS`
	...
	TroubleshootCPUDesc  = "4RedTeam requires at least 2 CPU cores for proper operation."
...
const (
	// Form interface implementation
	EULAFormDescription = "Legal terms and conditions for 4RedTeam usage"
	EULAFormName        = "EULA"
	EULAFormOverview    = `Review and accept the End User License Agreement to proceed with 4RedTeam installation.
...
const (
	MainMenuTitle       = "4RedTeam Configuration"
	MainMenuDescription = "Configure all 4RedTeam components and settings"
	MainMenuName        = "Main Menu"
	MainMenuOverview    = `Welcome to 4RedTeam Configuration Center.
...
Navigate through each section to complete your 4RedTeam setup.`
...
const (
	MainMenuStatusRedteamRunning     = "4RedTeam is already running"
	MainMenuStatusRedteamNotRunning  = "Ready to start 4RedTeam services"
	MainMenuStatusUpToDate           = "4RedTeam is up to date"
...
const (
	...
	LLMProvidersOverview    = `4RedTeam uses specialized AI agents (researcher, developer, executor, pentester) that require different LLM capabilities for optimal penetration testing results.
...
Ready-to-use configurations for OpenRouter, DeepInfra, vLLM, Ollama, and other providers are available in the /opt/4redteam/conf/ directory inside the container`
)
...
Examples available: Pre-configured setups for major providers in /opt/4redteam/conf/ directory inside the container`
)
...
Monitor 4RedTeam performance and system health.`
...
const (
	...
	ServerSettingsFormDescription = "Configure 4RedTeam server network access and public routing"
	ServerSettingsFormName        = "Server Settings"
	ServerSettingsFormOverview    = `• Network binding - control which interface and port 4RedTeam listens on
...
• Data directory - persistent storage for agent artifacts and flow workspaces`
	...
	ServerSettingsLicenseKeyDesc = "4RedTeam License Key in format of XXXX-XXXX-XXXX-XXXX"
	...
	ServerSettingsPortDesc = "External TCP port exposed by Docker for 4RedTeam web UI"

	ServerSettingsPublicURL     = "Public URL"
	ServerSettingsPublicURLDesc = "Base public URL for redirects and links (supports base path, e.g., https://example.com/4redteam/)"
	...
	ServerSettingsExternalSSLCAPathDesc = "Path inside container to custom root CA cert (e.g., /opt/4redteam/ssl/ca-bundle.pem)"
	...
	ServerSettingsGeneralHelp = `4RedTeam exposes its web UI via Docker with configurable host and port.

Public URL must reflect how users reach the server. If using a subpath (e.g., /4redteam/), include it here. CORS controls browser access from specified origins. Proxy affects outbound traffic to LLM/search providers and other external services used by Tools.
...
SSL directory allows providing custom certificates. When set, server will use server.crt and server.key from that directory. Data directory stores artifacts and working files for flows.`
	...
	ServerSettingsLicenseKeyHelp = `4RedTeam License Key in format of XXXX-XXXX-XXXX-XXXX. It's used to communicate with 4RedTeam Cloud API.`
...
• 0.0.0.0 — expose on all interfaces`
	...
	ServerSettingsPortHelp = `External port for 4RedTeam UI. Must be available on the host. Example: 8443.`
...
• http://localhost:8443
• https://example.com/
• https://example.com/4redteam/ (with base path)`
...
Must point to /opt/4redteam/ssl/ directory, which is mounted from 4redteam-ssl volume on the host.

Examples:
• /opt/4redteam/ssl/ca-bundle.pem
• /opt/4redteam/ssl/corporate-ca.pem
...
When enabled, all certificate validation is bypassed, making connections vulnerable to man-in-the-middle attacks.`
	...
	ServerSettingsDataDirHelp = `Host directory for persistent data. 4RedTeam stores agent artifacts under flow-N subdirectories, which map to /work inside worker containers.`
...
const (
	...
	ToolsAIAgentsSettingsFormOverview    = `This section configures global behavior of AI agents across 4RedTeam.
...
const (
	...
	ToolsScraperFormOverview    = `Web scraper service for content extraction and analysis.
...
Recommended images include Kali Linux, Parrot Security, or custom security-focused containers. Default: jaysteelmind/kali-redteam`
...
Change providers carefully - different embedders produce incompatible vectors requiring database reindexing.`
	...
	EmbedderHelpGeneral = `Embeddings convert text into vectors for semantic search and knowledge storage. This enables 4RedTeam to understand meaning rather than just keywords, making search results more relevant and intelligent.
...
const (
	...
	ApplyChangesFormOverview = `This screen allows you to review all pending configuration changes and apply them to your 4RedTeam installation.
...
• Install additional components if needed`
	...
	ApplyChangesInstallNotFound = `4RedTeam is not currently installed on this system.

The following actions will be performed:
• Docker environment setup and validation
• Creation of docker-compose.yml file
• Installation and startup of 4RedTeam core services`
	...
	ApplyChangesUpdateFound = `4RedTeam is currently installed on this system.
...
const (
	...
	MaintenanceDescription = "Manage 4RedTeam services and perform maintenance operations"
	MaintenanceName        = "Maintenance"
	MaintenanceOverview    = `Perform system maintenance operations for 4RedTeam.
...
Each operation will provide real-time status updates and confirmation when required.`
	...
	MaintenanceStartRedteam            = "Start 4RedTeam"
	MaintenanceStartRedteamDesc        = "Start all configured 4RedTeam services"
	MaintenanceStopRedteam            = "Stop 4RedTeam"
	MaintenanceStopRedteamDesc         = "Stop all running 4RedTeam services"
	MaintenanceRestartRedteam         = "Restart 4RedTeam"
	MaintenanceRestartRedteamDesc      = "Restart all 4RedTeam services"
	...
	MaintenanceUpdateRedteam          = "Update 4RedTeam"
	MaintenanceUpdateRedteamDesc       = "Update 4RedTeam to the latest version"
	...
	MaintenanceFactoryResetDesc        = "Reset 4RedTeam to factory defaults"
	MaintenanceRemoveRedteam          = "Remove 4RedTeam"
	MaintenanceRemoveRedteamDesc       = "Remove 4RedTeam containers but keep data"
	MaintenancePurgeRedteam           = "Purge 4RedTeam"
	MaintenancePurgeRedteamDesc        = "Completely remove 4RedTeam including all data"
	MaintenanceResetPassword           = "Reset Admin Password"
	MaintenanceResetPasswordDesc       = "Reset the administrator password for 4RedTeam"
)

// Reset Password Screen constants
const (
	ResetPasswordFormTitle       = "Reset Admin Password"
	ResetPasswordFormDescription = "Reset the administrator password for 4RedTeam"
	ResetPasswordFormName        = "Reset Password"
	ResetPasswordFormOverview    = `Reset the password for the default administrator account (admin@example.com).

This operation requires 4RedTeam to be running and will update the password in the PostgreSQL database.
...
• Both password fields must match`
	...
	// Status messages
	ResetPasswordNotAvailable = "4RedTeam must be running to reset password"
	...
	// Help content
	ResetPasswordHelpContent = `Reset the administrator password for accessing 4RedTeam.

This operation:
• Updates the password for admin@example.com account
• Sets the user status to 'active'
• Requires 4RedTeam database to be accessible
...
const (
	...
	ProcessorOperationInstalling  = "Installing 4RedTeam services..."
	...
	OperationTitleInstallRedteam    = "Install 4RedTeam"
	OperationDescInstallRedteam     = "Install and configure 4RedTeam services"
...
const (
	ProcessorHelpInstallRedteam = `This will:
...
Installation will use your current configuration settings.`
	...
	ProcessorHelpStartRedteam = `This will:
• Core 4RedTeam API and web interface
...
Services will be started in the correct dependency order.`
	...
	ProcessorHelpStopRedteam = `This will:
...
You can restart services later without losing any data.`
	...
	ProcessorHelpRestartRedteam = `This will:
...
Note: This is a large download (6GB+).`
	...
	ProcessorHelpUpdateRedteam = `This will:
...
This action cannot be undone!`
	...
	ProcessorHelpRemoveRedteam = `This will:
...
You can reinstall later without losing data.`
	...
	ProcessorHelpPurgeRedteam = `⚠️  WARNING: This will permanently delete:
...
const (
	...
	EnvDesc_REDTEAM_LISTEN_IP                 = "4RedTeam Server Host"
	EnvDesc_REDTEAM_LISTEN_PORT               = "4RedTeam Server Port"
	EnvDesc_PUBLIC_URL                        = "4RedTeam Public URL"
	EnvDesc_CORS_ORIGINS                      = "4RedTeam CORS Origins"
	EnvDesc_COOKIE_SIGNING_SALT               = "4RedTeam Cookie Signing Salt"
	...
	EnvDesc_REDTEAM_SSL_DIR                   = "4RedTeam SSL Directory"
	EnvDesc_REDTEAM_DATA_DIR                  = "4RedTeam Data Directory"
	EnvDesc_REDTEAM_DOCKER_SOCKET             = "Mount Docker Socket Path"
	EnvDesc_REDTEAM_DOCKER_CERT_PATH          = "Mount Docker Certificate Path"
	EnvDesc_REDTEAM_LLM_SERVER_CONFIG_PATH    = "Custom LLM Host Config Path"
	EnvDesc_REDTEAM_OLLAMA_SERVER_CONFIG_PATH = "Ollama Host Config Path"
	...
	EnvDesc_REDTEAM_POSTGRES_PASSWORD = "4RedTeam PostgreSQL Password"
...
const (
	...
	// component labels
	ProcessorComponentRedteam       = "4RedTeam"
	...
	EffectsStart           = "4RedTeam web UI becomes available. Background services are brought online in the required order."
	EffectsStop            = "Web UI becomes unavailable. In-progress flows pause safely. When you start 4RedTeam again, flows resume automatically. A small portion of the current agent step may be lost."
