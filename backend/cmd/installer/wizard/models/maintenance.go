...
func (h *MaintenanceHandler) LoadItems() []ListItem {
	...
	if showStart {
		items = append(items, ListItem{
			ID: StartRedteamScreen,
		})
	}

	// stop 4RedTeam - show if any stack is running
	showStop := checker.CanStopAll()
	if showStop {
		items = append(items, ListItem{
			ID: StopRedteamScreen,
		})
	}

	// restart 4RedTeam - show if any stack is running
	if showStop {
		items = append(items, ListItem{
			ID: RestartRedteamScreen,
		})
	}
	...
	// update 4RedTeam - show if updates are available for any stack
	showUpdateRedteam := checker.CanUpdateAll()

	if showUpdateRedteam {
		items = append(items, ListItem{
			ID:          UpdateRedteamScreen,
			Highlighted: true,
		})
	}
	...
	// remove 4RedTeam - show if any stack is installed
	if checker.CanRemoveAll() {
		items = append(items, ListItem{
			ID: RemoveRedteamScreen,
		})
	}

	// purge 4RedTeam - show if any stack is installed
	if checker.CanPurgeAll() {
		items = append(items, ListItem{
			ID: PurgeRedteamScreen,
		})
	}

	// reset admin password - show if 4RedTeam is running
