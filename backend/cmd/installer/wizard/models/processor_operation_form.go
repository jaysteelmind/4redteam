...
func (m *ProcessorOperationFormModel) getOperationInfo() *processorOperationInfo {
	...
	case processor.ProcessorOperationStart:
		info.title = locale.MaintenanceStartRedteam
		info.description = locale.MaintenanceStartRedteamDesc
		info.help = locale.ProcessorHelpStartRedteam
		info.progressMessage = locale.ProcessorOperationStarting

	case processor.ProcessorOperationStop:
		info.title = locale.MaintenanceStopRedteam
		info.description = locale.MaintenanceStopRedteamDesc
		info.help = locale.ProcessorHelpStopRedteam
		info.progressMessage = locale.ProcessorOperationStopping

	case processor.ProcessorOperationRestart:
		info.title = locale.MaintenanceRestartRedteam
		info.description = locale.MaintenanceRestartRedteamDesc
		info.help = locale.ProcessorHelpRestartRedteam
	...
	case processor.ProcessorOperationUpdate:
		...
		case processor.ProductStackAll, processor.ProductStackCompose:
			info.title = locale.MaintenanceUpdateRedteam
			info.description = locale.MaintenanceUpdateRedteamDesc
			info.help = locale.ProcessorHelpUpdateRedteam
			info.requiresConfirmation = true
		default:
	...
	case processor.ProcessorOperationRemove:
		info.title = locale.MaintenanceRemoveRedteam
		info.description = locale.MaintenanceRemoveRedteamDesc
		info.help = locale.ProcessorHelpRemoveRedteam
		info.progressMessage = locale.ProcessorOperationRemoving

	case processor.ProcessorOperationPurge:
		info.title = locale.MaintenancePurgeRedteam
		info.description = locale.MaintenancePurgeRedteamDesc
		info.help = locale.ProcessorHelpPurgeRedteam
		info.progressMessage = locale.ProcessorOperationPurging
		info.requiresConfirmation = true

	case processor.ProcessorOperationInstall:
		info.title = locale.OperationTitleInstallRedteam
		info.description = locale.OperationDescInstallRedteam
		info.help = locale.ProcessorHelpInstallRedteam
...
func (m *ProcessorOperationFormModel) renderCurrentStateSummary() string {
	...
	lines = append(lines, comp(locale.ProcessorComponentRedteam, c.RedteamInstalled, c.RedteamRunning, true, true, false))
...
func (m *ProcessorOperationFormModel) renderPlannedActions() string {
	...
	case processor.ProcessorOperationStart:
		...
		add(locale.PlannedWillStart, locale.ProcessorComponentRedteam, c.CanStartAll() && !c.RedteamRunning)
	case processor.ProcessorOperationStop:
		add(locale.PlannedWillStop, locale.ProcessorComponentRedteam, c.RedteamRunning)
	...
	case processor.ProcessorOperationRestart:
		add(locale.PlannedWillRestart, locale.ProcessorComponentRedteam, c.RedteamRunning)
	...
	case processor.ProcessorOperationUpdate:
		...
		add(locale.PlannedWillUpdate, locale.ProcessorComponentRedteam, c.RedteamInstalled && !c.RedteamIsUpToDate)
		if !(c.RedteamInstalled || c.LangfuseInstalled || c.ObservabilityInstalled) {
	...
	case processor.ProcessorOperationInstall:
		...
		add(locale.PlannedWillStart, locale.ProcessorComponentRedteam, true)
	}
