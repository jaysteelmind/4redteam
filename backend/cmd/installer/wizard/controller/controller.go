...
import (
	...
	"4redteam/cmd/installer/checker"
	"4redteam/cmd/installer/files"
	"4redteam/cmd/installer/loader"
	"4redteam/cmd/installer/state"
	"4redteam/cmd/installer/wizard/locale"
)

const (
	EmbeddedLLMConfigsPath   = "providers-configs"
	DefaultDockerCertPath    = "/opt/4redteam/docker/ssl"
	DefaultCustomConfigsPath = "/opt/4redteam/conf/custom.provider.yml"
	DefaultOllamaConfigsPath = "/opt/4redteam/conf/ollama.provider.yml"
	DefaultLLMConfigsPath    = "/opt/4redteam/conf/"
...
type LLMProviderConfig struct {
	...
	HostConfigPath    loader.EnvVar // REDTEAM_OLLAMA_SERVER_CONFIG_PATH | REDTEAM_LLM_SERVER_CONFIG_PATH
...
func (c *controller) GetLLMProviderConfig(providerID string) *LLMProviderConfig {
	...
	case "ollama":
		...
		providerConfig.HostConfigPath, _ = c.GetVar("REDTEAM_OLLAMA_SERVER_CONFIG_PATH")
	...
	case "custom":
		...
		providerConfig.HostConfigPath, _ = c.GetVar("REDTEAM_LLM_SERVER_CONFIG_PATH")
		if slices.Contains(providersConfigsPath, providerConfig.ConfigPath.Value) {
			providerConfig.HostConfigPath.Value = providerConfig.ConfigPath.Value
		}
...
func (c *controller) ResetLLMProviderConfig(providerID string) map[string]*LLMProviderConfig {
	...
	case "ollama":
		vars = []string{
			...
			"REDTEAM_OLLAMA_SERVER_CONFIG_PATH",
		}
	case "custom":
		vars = []string{
			...
			"REDTEAM_LLM_SERVER_CONFIG_PATH", // local path to the LLM config file
		}
	}
...
func (c *controller) GetEmbedderConfig() *EmbedderConfig {
	...
	config.Installed = c.checker.RedteamInstalled
...
type DockerConfig struct {
	...
	HostDockerCertPath loader.EnvVar // REDTEAM_DOCKER_CERT_PATH
...
func (c *controller) GetDockerConfig() *DockerConfig {
	...
	config := &DockerConfig{
		...
		HostDockerCertPath:           vars["REDTEAM_DOCKER_CERT_PATH"],
	}
...
func (c *controller) UpdateDockerConfig(config *DockerConfig) error {
	updates := map[string]string{
		...
		"REDTEAM_DOCKER_CERT_PATH":         config.HostDockerCertPath.Value,
	}

	dockerHost := config.DockerHost.Value
	if strings.HasPrefix(dockerHost, "unix://") && !config.DockerHost.IsDefault() {
		// mount custom docker socket to the 4redteam container
		updates["REDTEAM_DOCKER_SOCKET"] = strings.TrimPrefix(dockerHost, "unix://")
	} else {
		// ensure previous custom socket mapping is cleared when not using unix socket
		updates["REDTEAM_DOCKER_SOCKET"] = ""
	}
...
// ServerSettingsConfig represents 4RedTeam server settings configuration
type ServerSettingsConfig struct {
	// direct form field mappings using loader.EnvVar
	ListenIP            loader.EnvVar // REDTEAM_LISTEN_IP
	ListenPort          loader.EnvVar // REDTEAM_LISTEN_PORT
	...
	SSLDir              loader.EnvVar // REDTEAM_SSL_DIR
	DataDir             loader.EnvVar // REDTEAM_DATA_DIR
...
func (c *controller) GetServerSettingsConfig() *ServerSettingsConfig {
	...
	defaults := map[string]string{
		...
		"REDTEAM_DATA_DIR":      "redteam-data",
		"REDTEAM_SSL_DIR":       "redteam-ssl",
	...
	cfg := &ServerSettingsConfig{
		ListenIP:            vars["REDTEAM_LISTEN_IP"],
		ListenPort:          vars["REDTEAM_LISTEN_PORT"],
		...
		SSLDir:              vars["REDTEAM_SSL_DIR"],
		DataDir:             vars["REDTEAM_DATA_DIR"],
	}
...
func (c *controller) UpdateServerSettingsConfig(config *ServerSettingsConfig) error {
	...
	updates := map[string]string{
		"REDTEAM_LISTEN_IP":     config.ListenIP.Value,
		"REDTEAM_LISTEN_PORT":   config.ListenPort.Value,
		...
		"REDTEAM_SSL_DIR":       config.SSLDir.Value,
		"REDTEAM_DATA_DIR":      config.DataDir.Value,
	}
...
type ApplyChangesConfig struct {
	// installation state
	IsInstalled bool // whether 4RedTeam is currently installed
...
func (c *controller) GetApplyChangesConfig() *ApplyChangesConfig {
	config := &ApplyChangesConfig{
		IsInstalled: c.checker.RedteamInstalled,
		Changes:     []ChangeInfo{},
	}
...
func (c *controller) getVariableDescription(varName string) string {
	...
	envVarDescriptions := map[string]string{
		...
		"REDTEAM_LISTEN_IP":                 locale.EnvDesc_REDTEAM_LISTEN_IP,
		"REDTEAM_LISTEN_PORT":               locale.EnvDesc_REDTEAM_LISTEN_PORT,
		...
		"REDTEAM_SSL_DIR":                   locale.EnvDesc_REDTEAM_SSL_DIR,
		"REDTEAM_DATA_DIR":                  locale.EnvDesc_REDTEAM_DATA_DIR,
		"REDTEAM_DOCKER_SOCKET":             locale.EnvDesc_REDTEAM_DOCKER_SOCKET,
		"REDTEAM_DOCKER_CERT_PATH":          locale.EnvDesc_REDTEAM_DOCKER_CERT_PATH,
		"REDTEAM_LLM_SERVER_CONFIG_PATH":    locale.EnvDesc_REDTEAM_LLM_SERVER_CONFIG_PATH,
		"REDTEAM_OLLAMA_SERVER_CONFIG_PATH": locale.EnvDesc_REDTEAM_OLLAMA_SERVER_CONFIG_PATH,
		...
		"REDTEAM_POSTGRES_PASSWORD": locale.EnvDesc_REDTEAM_POSTGRES_PASSWORD,
...
var maskedVariables = map[string]bool{
	...
	// postgres password for 4redteam service (pgvector binds on localhost)
	"REDTEAM_POSTGRES_PASSWORD": true,
...
var criticalVariables = map[string]bool{
	...
	// mounting custom LLM server config into 4redteam container changes volume mapping
	"REDTEAM_LLM_SERVER_CONFIG_PATH":    true,
	"REDTEAM_OLLAMA_SERVER_CONFIG_PATH": true,
	...
	"ASK_USER":              true,
	"REDTEAM_LISTEN_IP":     true,
	"REDTEAM_LISTEN_PORT":   true,
	"PUBLIC_URL":            true,
	"CORS_ORIGINS":          true,
	"COOKIE_SIGNING_SALT":   true,
	"PROXY_URL":             true,
	...
	"SERVER_SSL_KEY":        true,
	"SERVER_USE_SSL":        true,
	"REDTEAM_SSL_DIR":       true,
	"REDTEAM_DATA_DIR":      true,
	...
	// langfuse integration settings passed to 4redteam
