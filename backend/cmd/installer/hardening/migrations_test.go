...
func TestDoMigrateSettings_SuccessfulMigrations(t *testing.T) {
	tests := []struct {
		...
		{
			name: "migrate DOCKER_CERT_PATH to REDTEAM_DOCKER_CERT_PATH",
			...
			redteamVarName:  "REDTEAM_DOCKER_CERT_PATH",
			defaultPath:     controller.DefaultDockerCertPath,
			pathType:        directory,
			expectMigration: true,
		},
		{
			name: "migrate LLM_SERVER_CONFIG_PATH to REDTEAM_LLM_SERVER_CONFIG_PATH",
			setupFunc: func(t *testing.T) (string, func()) {
			...
			redteamVarName:  "REDTEAM_LLM_SERVER_CONFIG_PATH",
			defaultPath:     controller.DefaultCustomConfigsPath,
			pathType:        file,
			expectMigration: true,
		},
		{
			name: "migrate OLLAMA_SERVER_CONFIG_PATH to REDTEAM_OLLAMA_SERVER_CONFIG_PATH",
			setupFunc: func(t *testing.T) (string, func()) {
			...
			redteamVarName:  "REDTEAM_OLLAMA_SERVER_CONFIG_PATH",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectMigration {
				// check that REDTEAM_* variable was set to custom path
				redteamVar, exists := mockSt.GetVar(tt.redteamVarName)
				if !exists {
					t.Errorf("Expected %s to be set", tt.redteamVarName)
				} else if redteamVar.Value != customPath {
					t.Errorf("Expected %s = %q, got %q", tt.redteamVarName, customPath, redteamVar.Value)
				}
...
func TestDoMigrateSettings_VariableNotSet(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, exists := mockSt.GetVar(tt.redteamVarName)
			if exists {
				t.Errorf("Expected %s to not be set", tt.redteamVarName)
			}
		})
	}
}
...
func TestDoMigrateSettings_PathNotExist(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, exists := mockSt.GetVar(tt.redteamVarName)
			if exists {
				t.Errorf("Expected %s to not be set for non-existing path", tt.redteamVarName)
			}
		})
	}
}
...
func TestDoMigrateSettings_AlreadyDefaultValue(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, exists := mockSt.GetVar(tt.redteamVarName)
			if exists {
				t.Errorf("Expected %s to not be set when already using default", tt.redteamVarName)
			}
...
func TestDoMigrateSettings_EmbeddedConfigs(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			varName:        "LLM_SERVER_CONFIG_PATH",
			redteamVarName: "REDTEAM_LLM_SERVER_CONFIG_PATH",
			embeddedPath:   "/opt/4redteam/conf/llms/openai.yml",
		...
		},
		{
			...
			varName:        "OLLAMA_SERVER_CONFIG_PATH",
			redteamVarName: "REDTEAM_OLLAMA_SERVER_CONFIG_PATH",
			embeddedPath:   "/opt/4redteam/conf/llms/llama3.yml",
			description:    "Embedded configs are inside docker image, no migration needed",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// verify no migration occurred
			redteamVar, exists := mockSt.GetVar(tt.redteamVarName)
			if exists && redteamVar.Value != "" {
				t.Errorf("Expected %s to not be set for embedded config: %s", tt.redteamVarName, tt.description)
			}
...
func TestDoMigrateSettings_WrongPathType(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, exists := mockSt.GetVar(tt.redteamVarName)
			if exists {
				t.Errorf("Expected %s to not be set for wrong path type: %s", tt.redteamVarName, tt.description)
			}
		})
	}
}
...
func TestDoMigrateSettings_ErrorHandling(t *testing.T) {
	tests := []struct {
		...
		{
			name: "SetVar error for REDTEAM_DOCKER_CERT_PATH",
			setupFunc: func(t *testing.T) (*mockStateWithErrors, string, func()) {
				...
				mockSt := &mockStateWithErrors{
					...
					setVarError: map[string]error{
						"REDTEAM_DOCKER_CERT_PATH": mockError,
					},
				}
		...
		{
			name: "SetVar error for REDTEAM_LLM_SERVER_CONFIG_PATH",
			setupFunc: func(t *testing.T) (*mockStateWithErrors, string, func()) {
				tmpFile, err := os.CreateTemp("", "custom-*.yml")
				if err != nil {
					t.Fatalf("Failed to create temp file: %v", err)
				...
				mockSt := &mockStateWithErrors{
					...
					setVarError: map[string]error{
						"REDTEAM_LLM_SERVER_CONFIG_PATH": mockError,
					},
				}
		...
		{
			name: "SetVar error for REDTEAM_OLLAMA_SERVER_CONFIG_PATH",
			setupFunc: func(t *testing.T) (*mockStateWithErrors, string, func()) {
				tmpFile, err := os.CreateTemp("", "ollama-*.yml")
				if err != nil {
					t.Fatalf("Failed to create temp file: %v", err)
				...
				mockSt := &mockStateWithErrors{
					...
					setVarError: map[string]error{
						"REDTEAM_OLLAMA_SERVER_CONFIG_PATH": mockError,
					},
				}
				return mockSt, tmpFile.Name(), func() { os.Remove(tmpFile.Name()) }
			},
...
func TestDoMigrateSettings_CombinedMigrations(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectedVars: map[string]string{
				...
				// REDTEAM_* vars will be checked separately as they contain dynamic temp paths
			},
		...
		},
		{
			...
			setupFunc: func(t *testing.T) (map[string]string, func()) {
				...
				paths := map[string]string{
					"DOCKER_CERT_PATH":          dockerCertDir,
					"LLM_SERVER_CONFIG_PATH":    "/opt/4redteam/conf/llms/openai.yml", // embedded config
					"OLLAMA_SERVER_CONFIG_PATH": "/opt/4redteam/conf/llms/llama3.yml", // embedded config
				}
			...
			expectedVars: map[string]string{
				"DOCKER_CERT_PATH":          controller.DefaultDockerCertPath,
				"LLM_SERVER_CONFIG_PATH":    "/opt/4redteam/conf/llms/openai.yml", // should not change
				"OLLAMA_SERVER_CONFIG_PATH": "/opt/4redteam/conf/llms/llama3.yml", // should not change
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// verify REDTEAM_* variables were set correctly for non-default and non-embedded values
			for varName, originalValue := range paths {
				...
				case "LLM_SERVER_CONFIG_PATH":
					...
					isEmbedded = strings.HasPrefix(originalValue, "/opt/4redteam/conf/llms/")
				...
				if shouldMigrate {
					...
					if err == nil {
						...
						redteamVar, exists := mockSt.GetVar(redteamVarName)
						if !exists {
							t.Errorf("Expected %s to be set for non-default value", redteamVarName)
						} else if redteamVar.Value != originalValue {
							t.Errorf("Expected %s = %q, got %q", redteamVarName, originalValue, redteamVar.Value)
						}
					}
				} else {
					...
					redteamVar, exists := mockSt.GetVar(redteamVarName)
					if exists && redteamVar.Value != "" {
						t.Errorf("Expected %s to not be set for default/embedded value, but got %q", redteamVarName, redteamVar.Value)
					}
				}
			}
		})
