...
const (
	HardeningAreaRedteam  HardeningArea = "redteam"
...
var varsHardeningPolicies = map[HardeningArea]map[string]HardeningPolicy{
	HardeningAreaRedteam: {
		...
		"REDTEAM_POSTGRES_PASSWORD": {Type: HardeningPolicyTypeDefault, Length: 18},
...
func DoHardening(s state.State, c checker.CheckResult) error {
	...
	if id, _ := s.GetVar("INSTALLATION_ID"); id.Value == "" {
		if err := s.SetVar("INSTALLATION_ID", uuid.New().String()); err != nil {
	...
	// harden redteam vars only if neither containers nor volumes exist
	// this prevents password changes when volumes with existing credentials are present
	if vars, _ := s.GetVars(varsForHardening[HardeningAreaRedteam]); !c.RedteamInstalled && !c.RedteamVolumesExist {
		updateDefaultValues(vars)

		if isChanged, err := replaceDefaultValues(s, vars, varsHardeningPolicies[HardeningAreaRedteam]); err != nil {
			return fmt.Errorf("failed to replace default values for redteam: %w", err)
		} else if isChanged {
			haveToCommit = true
		}
