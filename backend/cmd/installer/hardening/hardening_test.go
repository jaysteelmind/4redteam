...
func TestUpdateDefaultValues(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			vars: map[string]loader.EnvVar{
				"REDTEAM_POSTGRES_PASSWORD":  {Name: "REDTEAM_POSTGRES_PASSWORD", Default: ""},
				"LANGFUSE_POSTGRES_PASSWORD": {Name: "LANGFUSE_POSTGRES_PASSWORD", Default: "custom"},
			},
			expected: map[string]string{
				"REDTEAM_POSTGRES_PASSWORD":  "postgres",
...
func TestDoHardening(t *testing.T) {
	tests := []struct {
		...
		{
			name: "redteam not installed - should harden",
			checkResult: checker.CheckResult{
		...
		{
			name: "redteam volumes exist but containers removed - should NOT harden",
			checkResult: checker.CheckResult{
				LangfuseInstalled:    true,
				LangfuseVolumesExist: true,
				GraphitiInstalled:    true,
				GraphitiVolumesExist: true,
				RedteamInstalled:     false, // containers removed
				RedteamVolumesExist:  true,  // but volumes remain!
			},
			setupVars: map[string]loader.EnvVar{
				"REDTEAM_POSTGRES_PASSWORD": {Name: "REDTEAM_POSTGRES_PASSWORD", Value: "postgres", Default: "postgres"},
			},
			expectChanges: false, // should NOT change because volumes exist
		},
		{
		...
		},
		{
			...
			checkResult: checker.CheckResult{
				...
				RedteamInstalled:     false, // containers removed
				RedteamVolumesExist:  true,  // volumes remain
			},
			setupVars: map[string]loader.EnvVar{
				"LANGFUSE_SALT":             {Name: "LANGFUSE_SALT", Value: "salt", Default: "salt"},
				"NEO4J_PASSWORD":            {Name: "NEO4J_PASSWORD", Value: "devpassword", Default: "devpassword"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			checkResult := checker.CheckResult{
				LangfuseInstalled:    true,
				LangfuseVolumesExist: true,
				RedteamInstalled:     false, // Trigger redteam hardening
				RedteamVolumesExist:  false,
			}
...
func TestDoHardening_IntegrationWithRealEnvFile(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			checkResult: checker.CheckResult{
				...
				GraphitiVolumesExist: true,
				RedteamInstalled:     true, // Should not harden redteam
				RedteamVolumesExist:  true,
			},
		...
		{
			name: "harden redteam only",
			checkResult: checker.CheckResult{
				LangfuseInstalled:    true, // Should not harden langfuse
				LangfuseVolumesExist: true,
				GraphitiInstalled:    true, // Should not harden graphiti
				GraphitiVolumesExist: true,
				RedteamInstalled:     false, // Should harden redteam
				RedteamVolumesExist:  false,
			},
			expectedHardenedVars: []string{
				"COOKIE_SIGNING_SALT",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Verify scraper URL consistency for Redteam
			if !tt.checkResult.RedteamInstalled {
