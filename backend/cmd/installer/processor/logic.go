...
func (p *processor) validateOperation(stack ProductStack, operation ProcessorOperation) error {
	...
	case ProcessorOperationResetPassword:
		if stack != ProductStackRedteam {
			return fmt.Errorf("operation %s only applicable for 4RedTeam stack", operation)
		}
	}
...
func (p *processor) isEmbeddedDeployment(stack ProductStack) bool {
	...
	case ProductStackRedteam, ProductStackWorker, ProductStackInstaller:
		return true
...
func (p *processor) applyChanges(ctx context.Context, state *operationState) (err error) {
	...
	// phase 4: 4RedTeam Stack Management (always embedded, always required)
	if err := p.applyRedteamChanges(ctx, state); err != nil {
		return fmt.Errorf("failed to apply 4redteam changes: %w", err)
	}
...
func (p *processor) applyRedteamChanges(ctx context.Context, state *operationState) error {
	// 4RedTeam is always embedded, always required
	if !p.checker.RedteamExtracted {
		// fresh installation - extract compose file
		if err := p.fsOps.ensureStackIntegrity(ctx, ProductStackRedteam, state); err != nil {
			return fmt.Errorf("failed to ensure 4redteam integrity: %w", err)
		}
	} else {
		// file exists - verify integrity, update if force=true
		if err := p.fsOps.verifyStackIntegrity(ctx, ProductStackRedteam, state); err != nil {
			return fmt.Errorf("failed to verify 4redteam integrity: %w", err)
		}
	}

	// update/start containers
	if err := p.composeOps.updateStack(ctx, ProductStackRedteam, state); err != nil {
		return fmt.Errorf("failed to update 4redteam stack: %w", err)
	}

	// refresh state to verify operation success
	if err := p.checker.GatherRedteamInfo(ctx); err != nil {
		return fmt.Errorf("failed to gather 4redteam info: %w", err)
	}
...
func (p *processor) checkFiles(
	...
	switch stack {
	case ProductStackRedteam, ProductStackGraphiti, ProductStackLangfuse, ProductStackObservability:
...
func (p *processor) factoryReset(ctx context.Context, state *operationState) (err error) {
	...
	_ = p.dockerOps.removeMainDockerNetwork(ctx, state, string(ProductDockerNetworkRedteam))
...
func (p *processor) update(ctx context.Context, stack ProductStack, state *operationState) (err error) {
	...
	composeStacksUpToDate := map[ProductStack]bool{
		ProductStackRedteam:       p.checker.RedteamIsUpToDate,
	...
	composeStacksGatherInfo := map[ProductStack]func(ctx context.Context) error{
		ProductStackRedteam:       p.checker.GatherRedteamInfo,
...
func (p *processor) resetPassword(ctx context.Context, stack ProductStack, state *operationState) (err error) {
	...
	if stack != ProductStackRedteam {
		return fmt.Errorf("reset password operation only supported for 4RedTeam stack")
	}

	if !p.checker.RedteamRunning {
		return fmt.Errorf("4RedTeam must be running to reset password")
	}
