...
const (
	...
	redteamExampleCustomConfigLLM = "example.custom.provider.yml"
	redteamExampleOllamaConfigLLM = "example.ollama.provider.yml"
)

var filesToExcludeFromVerification = []string{
	"observability/otel/config.yml",
	"observability/grafana/config/grafana.ini",
	redteamExampleCustomConfigLLM,
	redteamExampleOllamaConfigLLM,
}
...
func (fs *fileSystemOperationsImpl) ensureStackIntegrity(ctx context.Context, stack ProductStack, state *operationState) error {
	...
	switch stack {
	case ProductStackRedteam:
		errCompose := fs.ensureFileFromEmbed(composeFileRedteam, state)
		errCustom := fs.ensureFileFromEmbed(redteamExampleCustomConfigLLM, state)
		errOllama := fs.ensureFileFromEmbed(redteamExampleOllamaConfigLLM, state)
...
func (fs *fileSystemOperationsImpl) verifyStackIntegrity(ctx context.Context, stack ProductStack, state *operationState) error {
	...
	switch stack {
	case ProductStackRedteam:
		return fs.verifyFileIntegrity(composeFileRedteam, state)
...
func (fs *fileSystemOperationsImpl) checkStackIntegrity(ctx context.Context, stack ProductStack) (FilesCheckResult, error) {
	...
	switch stack {
	case ProductStackRedteam:
		result[composeFileRedteam] = fs.checkFileIntegrity(composeFileRedteam)
...
func (fs *fileSystemOperationsImpl) cleanupStackFiles(ctx context.Context, stack ProductStack, state *operationState) error {
	...
	switch stack {
	case ProductStackRedteam:
		filesToRemove = append(filesToRemove, filepath.Join(workingDir, composeFileRedteam))
		filesToRemove = append(filesToRemove, filepath.Join(workingDir, redteamExampleCustomConfigLLM))
		filesToRemove = append(filesToRemove, filepath.Join(workingDir, redteamExampleOllamaConfigLLM))
