...
func testStackOperation(t *testing.T,
	...
	t.Run("delegates_to_compose", func(t *testing.T) {
		...
		err := operation(p, t.Context(), ProductStackRedteam, testOperationState(t))
		assertNoError(t, err)
		...
		if calls[0].Method != expectedMethod || calls[0].Stack != ProductStackRedteam {
...
func TestUpdate(t *testing.T) {
	t.Run("compose_stacks", func(t *testing.T) {
		...
		testCases := []struct {
			...
			{
				name:         "redteam_needs_update",
				stack:        ProductStackRedteam,
				isUpToDate:   false,
				expectUpdate: true,
				configSetup: func(config *mockCheckConfig) {
					config.RedteamIsUpToDate = false
				},
			},
			{
				name:         "redteam_already_updated",
				stack:        ProductStackRedteam,
				isUpToDate:   true,
				expectUpdate: false,
				configSetup: func(config *mockCheckConfig) {
					config.RedteamIsUpToDate = true
				},
			},
			{
	...
	t.Run("compose_stacks", func(t *testing.T) {
		p, composeOps, _, dockerOps := newProcessorForLogicTestsWithConfig(t, func(config *mockCheckConfig) {
			config.RedteamIsUpToDate = true // should skip
			config.LangfuseIsUpToDate = false
		...
		// Check compose calls - should update langfuse and observability, skip redteam
		composeCalls := composeOps.getCalls()
		updateCount := 0
		for _, call := range composeCalls {
			if call.Method == "updateStack" {
				updateCount++
				// Verify we don't update redteam
				if call.Stack == ProductStackRedteam {
					t.Error("should not update redteam when it's up to date")
				}
			}
		}
		if updateCount != 2 {
	...
	t.Run("all_stacks", func(t *testing.T) {
		...
		// Check compose calls - should update redteam and observability, skip langfuse
		composeCalls := composeOps.getCalls()
		updateCount := 0
		for _, call := range composeCalls {
			if call.Method == "updateStack" {
...
func TestApplyChanges_ErrorPropagation_FromEnsureNetworks(t *testing.T) {
	...
	p.checker.RedteamExtracted = false
...
func TestPurge_StrictAndDockerCleanup(t *testing.T) {
	t.Run("compose_stack_purge", func(t *testing.T) {
		...
		err := p.purge(t.Context(), ProductStackRedteam, testOperationState(t))
		assertNoError(t, err)

		// first call must be strict purge (images)
		composeCalls := composeOps.getCalls()
		if len(composeCalls) == 0 || composeCalls[0].Method != "purgeImagesStack" || composeCalls[0].Stack != ProductStackRedteam {
			t.Fatalf("expected purgeImagesStack call for redteam, got: %+v", composeCalls)
		}
...
func TestApplyChanges_Embedded_AllStacksUpdated(t *testing.T) {
	...
	// since all not extracted -> ensure called for obs, langfuse, graphiti, redteam
...
func TestDownload_ComposeStacks(t *testing.T) {
	...
	expectedComposeStacks := []ProductStack{
		ProductStackRedteam, ProductStackGraphiti, ProductStackLangfuse, ProductStackObservability,
	}
	composeCallCount := 0
	for _, call := range composeCalls {
...
func TestValidateOperation_ErrorCases(t *testing.T) {
	...
	tests := []struct {
		...
		{"valid start redteam", ProductStackRedteam, ProcessorOperationStart, false, ""},
...
func TestIsEmbeddedDeployment(t *testing.T) {
	tests := []struct {
		...
		{"redteam always embedded", ProductStackRedteam, "", "", false, false, true},     // redteam is always embedded
	...
	tests := []struct {
	...
	}{
		{
			...
			setupError: func(p *processor) {
				_ = p.state.SetVar("OTEL_HOST", checker.DefaultObservabilityEndpoint)
				_ = p.state.SetVar("REDTEAM_VERSION", "1.0.0") // make state dirty
		...
		{
			name: "redteam phase error",
			configSetup: func(config *mockCheckConfig) {
				config.RedteamExtracted = false
			},
			setupError: func(p *processor) {
				// make state dirty so applyChanges proceeds
				_ = p.state.SetVar("REDTEAM_VERSION", "1.0.0")
				injectFSError(p, map[string]error{
					"ensureStackIntegrity_redteam": fmt.Errorf("redteam error"),
					"ensureStackIntegrity":         fmt.Errorf("general error"), // fallback to catch any call
				})
			},
			expectedError: "failed to apply 4redteam changes: failed to ensure 4redteam integrity: redteam error",
		},
	}
...
func TestInstall_FullScenario(t *testing.T) {
	t.Run("all_stacks_fresh_install", func(t *testing.T) {
		...
		// should be 3 (observability, langfuse, graphiti) since redteam might be handled differently
		if ensureCount < 3 {
		...
		// all 4 stacks should be updated (observability, langfuse, graphiti, redteam)
		if updateCount != 4 {
	...
	t.Run("partial_install_skip_installed", func(t *testing.T) {
		p, composeOps, _, _ := newProcessorForLogicTestsWithConfig(t, func(config *mockCheckConfig) {
			// redteam already installed
			config.RedteamInstalled = true
			config.LangfuseInstalled = false
			config.ObservabilityInstalled = false
		})
		...
		// should not update redteam since it's already installed
		composeCalls := composeOps.getCalls()
		for _, call := range composeCalls {
			if call.Method == "updateStack" && call.Stack == ProductStackRedteam {
				t.Error("should not update redteam when already installed")
			}
		}
	})
}
...
func TestPreviewFilesStatus_Behavior(t *testing.T) {
	...
	mockFiles.statuses[composeFileRedteam] = files.FileStatusModified
	...
	// redteam, observability compose must be present and reflect modified
	for _, k := range []string{composeFileRedteam, composeFileObservability} {
...
func TestPurge_AllStacks_Detailed(t *testing.T) {
	...
	expectedOrder := []ProductStack{
		ProductStackObservability, ProductStackLangfuse, ProductStackGraphiti, ProductStackRedteam,
	}
	purgeImagesCalls := 0
	for _, call := range composeCalls {
...
func TestRemove_PreservesData(t *testing.T) {
	t.Run("compose_stacks_preserve_volumes", func(t *testing.T) {
		...
		stacks := []ProductStack{ProductStackRedteam, ProductStackLangfuse, ProductStackObservability}
...
func TestApplyChanges_ComplexScenarios(t *testing.T) {
	t.Run("mixed_deployment_modes", func(t *testing.T) {
		p, composeOps, _, _ := newProcessorForLogicTestsWithConfig(t, func(config *mockCheckConfig) {
			// observability external, langfuse embedded, graphiti disabled, redteam always embedded
