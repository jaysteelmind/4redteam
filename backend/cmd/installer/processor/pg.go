...
const (
	...
	DefaultPostgreSQLDatabase = "redteamdb"

	// Admin user email
	AdminEmail = "admin@example.com"

	// Environment variable names
	EnvPostgreSQLUser     = "REDTEAM_POSTGRES_USER"
	EnvPostgreSQLPassword = "REDTEAM_POSTGRES_PASSWORD"
	EnvPostgreSQLDatabase = "REDTEAM_POSTGRES_DB"
)
...
func (p *processor) performPasswordReset(ctx context.Context, newPassword string, state *operationState) error {
	...
	p.appendLog(fmt.Sprintf("Connected to PostgreSQL at %s:%s (database: %s)", PostgreSQLHost, PostgreSQLPort, dbName), ProductStackRedteam, state)
	...
	p.appendLog(fmt.Sprintf("Password updated for %s", AdminEmail), ProductStackRedteam, state)
