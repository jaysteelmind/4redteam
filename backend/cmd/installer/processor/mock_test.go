...
func newMockState() *mockState {
	dir, err := os.MkdirTemp("", "redteam-test")
	if err != nil {
		panic(err)
	}
...
func testState(t *testing.T) *mockState {
	...
	if _, err := os.Stat(envPath); os.IsNotExist(err) {
		if err := os.WriteFile(envPath, []byte("REDTEAM_VERSION=1.0.0\n"), 0o644); err != nil {
...
func defaultCheckResult() *checker.CheckResult {
	...
	handler.config.RedteamExtracted = false
	handler.config.RedteamInstalled = false
	handler.config.RedteamRunning = false
	...
	handler.config.RedteamIsUpToDate = true
...
func (h *defaultStateHandler) GatherRedteamInfo(ctx context.Context, c *checker.CheckResult) error {
	if !h.initialized {
		return h.mockHandler.GatherRedteamInfo(ctx, c)
	}
	return nil
}
...
type mockCheckConfig struct {
	...
	// 4RedTeam states
	RedteamScriptInstalled bool
	RedteamExtracted       bool
	RedteamInstalled       bool
	RedteamRunning         bool
...
func (m *mockCheckHandler) GatherAllInfo(ctx context.Context, c *checker.CheckResult) error {
	...
	if err := m.GatherRedteamInfo(ctx, c); err != nil {
		return err
	}
...
func (m *mockCheckHandler) GatherRedteamInfo(ctx context.Context, c *checker.CheckResult) error {
	m.recordCall("GatherRedteamInfo")
	if m.gatherError != nil {
		return m.gatherError
	}
	...
	c.RedteamScriptInstalled = m.config.RedteamScriptInstalled
	c.RedteamExtracted = m.config.RedteamExtracted
	c.RedteamInstalled = m.config.RedteamInstalled
	c.RedteamRunning = m.config.RedteamRunning
...
func (m *mockCheckHandler) GatherUpdatesInfo(ctx context.Context, c *checker.CheckResult) error {
	...
	c.RedteamIsUpToDate = m.config.RedteamIsUpToDate
...
func TestMockCheckHandler_BasicFunctionality(t *testing.T) {
	...
	err = handler.GatherRedteamInfo(ctx, result)
	assertNoError(t, err)

	// Verify calls were recorded
	calls := handler.getCalls()
	expectedCalls := []string{"GatherDockerInfo", "GatherWorkerInfo", "GatherRedteamInfo"}
...
func TestMockCheckHandler_CustomConfiguration(t *testing.T) {
	...
	customConfig := mockCheckConfig{
		RedteamExtracted:       false,
		RedteamInstalled:       true,
	...
	// Gather info
	ctx := context.Background()
	_ = handler.GatherRedteamInfo(ctx, result)
	_ = handler.GatherLangfuseInfo(ctx, result)
	_ = handler.GatherObservabilityInfo(ctx, result)

	// Verify custom values were applied
	if result.RedteamExtracted != false {
		t.Error("expected RedteamExtracted to be false")
	}
	if result.RedteamInstalled != true {
		t.Error("expected RedteamInstalled to be true")
	}
...
func TestMockOperations_CallAccumulation(t *testing.T) {
	t.Run("FileSystemOperations", func(t *testing.T) {
		...
		// make multiple calls
		_ = mock.ensureStackIntegrity(t.Context(), ProductStackRedteam, state)
		...
		expectedCalls := []struct {
			...
			{"ensureStackIntegrity", ProductStackRedteam},
...
func TestMockOperations_ErrorIsolation(t *testing.T) {
	t.Run("StackSpecificErrors", func(t *testing.T) {
		...
		mock.errOn["ensureStackIntegrity_"+string(ProductStackRedteam)] = fmt.Errorf("redteam-specific error")

		// redteam should fail
		err := mock.ensureStackIntegrity(t.Context(), ProductStackRedteam, state)
		if err == nil || err.Error() != "redteam-specific error" {
			t.Errorf("expected redteam-specific error, got %v", err)
		}
...
func TestMockState_ComplexOperations(t *testing.T) {
	...
	t.Run("StackManagement", func(t *testing.T) {
		stack := []string{"redteam", "langfuse", "observability"}
...
func TestMockCheckHandler_CompleteScenarios(t *testing.T) {
	t.Run("AllSystemsHealthy", func(t *testing.T) {
		...
		handler.config.RedteamInstalled = true
		handler.config.RedteamRunning = true
		...
		if !result.DockerApiAccessible || !result.WorkerImageExists ||
			!result.RedteamRunning || !result.LangfuseRunning ||
	...
	t.Run("PartialFailures", func(t *testing.T) {
		...
		if result.RedteamInstalled {
			t.Error("expected 4RedTeam not to be installed")
		}
...
func TestBaseMockComposeOperations_SpecialMethods(t *testing.T) {
	...
	t.Run("determineComposeFile", func(t *testing.T) {
		...
		file, err := mock.determineComposeFile(ProductStackRedteam)
		assertNoError(t, err)
		...
		calls := mock.getCalls()
		if len(calls) != 1 || calls[0].Method != "determineComposeFile" || calls[0].Stack != ProductStackRedteam {
			t.Fatalf("unexpected calls: %+v", calls)
		}
	...
	t.Run("performStackCommand", func(t *testing.T) {
		...
		err := mock.performStackCommand(t.Context(), ProductStackRedteam, state, args...)
		assertNoError(t, err)

		calls := mock.getCalls()
		// offset by 2 due to previous test
		if len(calls) != 3 || calls[2].Method != "performStackCommand" || calls[2].Stack != ProductStackRedteam {
			t.Fatalf("unexpected calls: %+v", calls)
		}
