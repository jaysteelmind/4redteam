...
> 🚀 **Join the Community!** Connect with security researchers, AI enthusiasts, and fellow ethical hackers. Get support, share insights, and stay updated with the latest 4RedTeam developments.
...
4RedTeam is an innovative tool for automated security testing that leverages cutting-edge artificial intelligence technologies. The project is designed for information security professionals, researchers, and enthusiasts who need a powerful and flexible solution for conducting penetration tests.

You can watch the video **4RedTeam overview**:
[![4RedTeam Overview Video](https://github.com/user-attachments/assets/0828dc3e-15f1-4a1d-858e-9696a146e478)](https://youtu.be/R70x5Ddzs1o)
...
- 🔍 Web Intelligence. Built-in browser via [scraper](https://hub.docker.com/r/jaysteelmind/scraper) for gathering latest information from web sources.
...
- 💾 Persistent Storage. All commands and outputs are stored in PostgreSQL with [pgvector](https://hub.docker.com/r/jaysteelmind/pgvector) extension.
...
flowchart TB
    ...
    pentester --> |Uses HTTPS| 4redteam
    pentester --> |Monitors AI HTTPS| langfuse
    pentester --> |Monitors System HTTPS| grafana
    4redteam --> |Tests Various protocols| target
    4redteam --> |Queries HTTPS| llm
    4redteam --> |Searches HTTPS| search
    4redteam --> |Reports HTTPS| langfuse
    4redteam --> |Reports HTTPS| grafana

    class pentester person
    class 4redteam system
...
The architecture of 4RedTeam is designed to be modular, scalable, and secure. Here are the key components:
...
4RedTeam provides an interactive installer with a terminal-based UI for streamlined configuration and deployment. The installer guides you through system checks, LLM provider setup, search engine configuration, and security hardening.

**Supported Platforms:**
- **Linux**: amd64 [download](https://example.com/downloads/linux/amd64/installer-latest.zip) | arm64 [download](https://example.com/downloads/linux/arm64/installer-latest.zip)
- **Windows**: amd64 [download](https://example.com/downloads/windows/amd64/installer-latest.zip)
- **macOS**: amd64 (Intel) [download](https://example.com/downloads/darwin/amd64/installer-latest.zip) | arm64 (M-series) [download](https://example.com/downloads/darwin/arm64/installer-latest.zip)

**Quick Installation (Linux amd64):**

```bash
# Create installation directory
mkdir -p 4redteam && cd 4redteam
...
6. **Deployment**: Start 4RedTeam with docker-compose
...
```bash
curl -o .env https://raw.githubusercontent.com/jaysteelmind/4redteam/master/.env.example
```

3. Touch examples files (`example.custom.provider.yml`, `example.ollama.provider.yml`) or download it:

```bash
curl -o example.custom.provider.yml https://raw.githubusercontent.com/jaysteelmind/4redteam/master/examples/configs/custom-openai.provider.yml
curl -o example.ollama.provider.yml https://raw.githubusercontent.com/jaysteelmind/4redteam/master/examples/configs/ollama-llama318b.provider.yml
```
...
- `PUBLIC_URL` - Public URL of your server (eg. `https://4redteam.example.com`)
...
### Access Credentials
- `REDTEAM_POSTGRES_USER` and `REDTEAM_POSTGRES_PASSWORD` - PostgreSQL credentials
...
7. Run the 4RedTeam stack:

```bash
curl -O https://raw.githubusercontent.com/jaysteelmind/4redteam/master/docker-compose.yml
docker compose up -d
```

Visit [localhost:8443](https://localhost:8443) to access 4RedTeam Web UI (default is `admin@example.com` / `admin`)

> [!NOTE]
> If you caught an error about `redteam-network` or `observability-network` or `langfuse-network` you need to run `docker-compose.yml` firstly to create these networks and after that run `docker-compose-langfuse.yml`, `docker-compose-graphiti.yml`, and `docker-compose-observability.yml` to use Langfuse, Graphiti, and Observability services.
>
> You have to set at least one Language Model provider (OpenAI, Anthropic, Gemini, AWS Bedrock, or Ollama) to use 4RedTeam. AWS Bedrock provides enterprise-grade access to multiple foundation models from leading AI companies, while Ollama provides zero-cost local inference if you have sufficient computational resources. Additional API keys for search engines are optional but recommended for better results.
>
...
>
> The `docker-compose.yml` file runs the 4RedTeam service as root user because it needs access to docker.sock for container management. If you're using TCP/IP network connection to Docker instead of socket file, you can remove root privileges and use the default `4redteam` user for better security.

### Assistant Configuration

4RedTeam allows you to configure default behavior for assistants:
...
4RedTeam supports Ollama for local LLM inference, providing zero-cost operation and enhanced privacy:
...
OLLAMA_SERVER_CONFIG_PATH=/opt/4redteam/conf/ollama-llama318b.provider.yml
```
...
4RedTeam requires models with larger context windows than the default Ollama configurations. You need to create custom models with increased `num_ctx` parameter through Modelfiles. While typical agent workflows consume around 64K tokens, 4RedTeam uses 110K context size for safety margin and handling complex penetration testing scenarios.
...
These custom models are referenced in the pre-built provider configuration files (`ollama-qwen332b-fp16-tc.provider.yml` and `ollama-qwq32b-fp16-tc.provider.yml`) that are included in the Docker image at `/opt/4redteam/conf/`.

### OpenAI Provider Configuration

4RedTeam supports OpenAI's advanced language models, including the latest reasoning-capable o-series models designed for complex analytical tasks:
...
4RedTeam integrates with Anthropic's Claude models, known for their exceptional safety, reasoning capabilities, and sophisticated understanding of complex security contexts:
...
4RedTeam supports Google's Gemini models through the Google AI API, offering state-of-the-art reasoning capabilities and multimodal features:
...
4RedTeam integrates with Amazon Bedrock, offering access to a wide range of foundation models from leading AI companies including Anthropic, AI21, Cohere, Meta, and Amazon's own models:
...
>
> The default 4RedTeam configuration for AWS Bedrock uses two primary models:
...
>
> 4RedTeam uses the **Amazon Bedrock Converse API** for model interactions, which requires models to support the following features:
...
>
> ⚠️ **Important**: Some models like AI21 Jurassic-2 and Cohere Command (Text) have **limited chat support** and may not work properly with 4RedTeam's multi-turn conversation workflows.
...
For advanced configuration options and detailed setup instructions, please visit our [documentation](https://docs.example.com).
...
### API Keys and Tokens
- `LANGFUSE_INIT_PROJECT_PUBLIC_KEY` - Project public key (used from 4RedTeam side too)
- `LANGFUSE_INIT_PROJECT_SECRET_KEY` - Project secret key (used from 4RedTeam side too)
...
2. Enable integration with Langfuse for 4RedTeam service in `.env` file.
...
```bash
curl -O https://raw.githubusercontent.com/jaysteelmind/4redteam/master/docker-compose-langfuse.yml
...
1. Enable integration with OpenTelemetry and all observability services for 4RedTeam in `.env` file.
...
```bash
curl -O https://raw.githubusercontent.com/jaysteelmind/4redteam/master/docker-compose-observability.yml
...
>
> ```bash
> alias 4redteam="docker compose -f docker-compose.yml -f docker-compose-langfuse.yml -f docker-compose-graphiti.yml -f docker-compose-observability.yml"
> alias 4redteam-up="docker compose -f docker-compose.yml -f docker-compose-langfuse.yml -f docker-compose-graphiti.yml -f docker-compose-observability.yml up -d"
> alias 4redteam-down="docker compose -f docker-compose.yml -f docker-compose-langfuse.yml -f docker-compose-graphiti.yml -f docker-compose-observability.yml down"
> ```

### Knowledge Graph Integration (Graphiti)

4RedTeam integrates with [Graphiti](https://github.com/jaysteelmind/4redteam-graphiti), a temporal knowledge graph system powered by Neo4j, to provide advanced semantic understanding and relationship tracking for AI agent operations. The jaysteelmind fork provides custom entity and edge types that are specific to pentesting purposes.
...
2. Run the Graphiti stack along with the main 4RedTeam services:

```bash
# Download the Graphiti compose file if needed
curl -O https://raw.githubusercontent.com/jaysteelmind/4redteam/master/docker-compose-graphiti.yml

# Start 4RedTeam with Graphiti
...
> [!NOTE]
> The Graphiti service is defined in `docker-compose-graphiti.yml` as a separate stack. You must run both compose files together to enable the knowledge graph functionality. The pre-built Docker image `jaysteelmind/graphiti:latest` is used by default.

#### What Gets Stored

When enabled, 4RedTeam automatically captures:
...
4RedTeam allows you to configure Docker image selection for executing various tasks. The system automatically chooses the most appropriate image based on the task type, but you can constrain this selection by specifying your preferred images:

| Variable                           | Default                | Description                                                 |
| ---------------------------------- | ---------------------- | ----------------------------------------------------------- |
| `DOCKER_DEFAULT_IMAGE`             | `debian:latest`        | Default Docker image for general tasks and ambiguous cases  |
| `DOCKER_DEFAULT_IMAGE_FOR_PENTEST` | `jaysteelmind/kali-linux` | Default Docker image for security/penetration testing tasks |
...
```bash
docker run --rm -v $(pwd):/src -w /src --network redteam-network -e DATABASE_URL="{URL}" sqlc/sqlc generate -f sqlc/sqlc.yml
```
...
- `DATABASE_URL` - PostgreSQL database URL (eg. `postgres://postgres:postgres@localhost:5432/redteamdb?sslmode=disable`)
...
- Run `go run cmd/4redteam/main.go` to start the server
...
4RedTeam includes a powerful utility called `ctester` for testing and validating LLM agent capabilities. This tool helps ensure your LLM provider configurations work correctly with different agent types, allowing you to optimize model selection for each specific agent role.
...
docker run --rm -v $(pwd)/.env:/opt/4redteam/.env jaysteelmind/4redteam /opt/4redteam/bin/ctester -verbose

# Test with your custom provider configuration
docker run --rm \
  -v $(pwd)/.env:/opt/4redteam/.env \
  -v $(pwd)/my-config.yml:/opt/4redteam/config.yml \
  jaysteelmind/4redteam /opt/4redteam/bin/ctester -config /opt/4redteam/config.yml -agents simple,primary_agent,coder -verbose

# Generate a detailed report
docker run --rm \
  -v $(pwd)/.env:/opt/4redteam/.env \
  -v $(pwd):/opt/4redteam/output \
  jaysteelmind/4redteam /opt/4redteam/bin/ctester -report /opt/4redteam/output/report.md
```
...
docker run --rm \
  -v $(pwd)/.env:/opt/4redteam/.env \
  jaysteelmind/4redteam /opt/4redteam/bin/ctester -config /opt/4redteam/conf/openrouter.provider.yml

# Test with DeepInfra configuration
docker run --rm \
  -v $(pwd)/.env:/opt/4redteam/.env \
  jaysteelmind/4redteam /opt/4redteam/bin/ctester -config /opt/4redteam/conf/deepinfra.provider.yml

# Test with DeepSeek configuration
docker run --rm \
  -v $(pwd)/.env:/opt/4redteam/.env \
  jaysteelmind/4redteam /opt/4redteam/bin/ctester -config /opt/4redteam/conf/deepseek.provider.yml

# Test with Moonshot configuration
docker run --rm \
  -v $(pwd)/.env:/opt/4redteam/.env \
  jaysteelmind/4redteam /opt/4redteam/bin/ctester -config /opt/4redteam/conf/moonshot.provider.yml

# Test with OpenAI configuration
docker run --rm \
  -v $(pwd)/.env:/opt/4redteam/.env \
  jaysteelmind/4redteam /opt/4redteam/bin/ctester -type openai

# Test with Anthropic configuration
docker run --rm \
  -v $(pwd)/.env:/opt/4redteam/.env \
  jaysteelmind/4redteam /opt/4redteam/bin/ctester -type anthropic

# Test with Gemini configuration
docker run --rm \
  -v $(pwd)/.env:/opt/4redteam/.env \
  jaysteelmind/4redteam /opt/4redteam/bin/ctester -type gemini

# Test with AWS Bedrock configuration
docker run --rm \
  -v $(pwd)/.env:/opt/4redteam/.env \
  jaysteelmind/4redteam /opt/4redteam/bin/ctester -type bedrock

# Test with Custom OpenAI configuration
docker run --rm \
  -v $(pwd)/.env:/opt/4redteam/.env \
  jaysteelmind/4redteam /opt/4redteam/bin/ctester -config /opt/4redteam/conf/custom-openai.provider.yml

# Test with Ollama configuration (local inference)
docker run --rm \
  -v $(pwd)/.env:/opt/4redteam/.env \
  jaysteelmind/4redteam /opt/4redteam/bin/ctester -config /opt/4redteam/conf/ollama-llama318b.provider.yml

# Test with Ollama Qwen3 32B configuration (requires custom model creation)
docker run --rm \
  -v $(pwd)/.env:/opt/4redteam/.env \
  jaysteelmind/4redteam /opt/4redteam/bin/ctester -config /opt/4redteam/conf/ollama-qwen332b-fp16-tc.provider.yml

# Test with Ollama QwQ 32B configuration (requires custom model creation and 71.3GB VRAM)
docker run --rm \
  -v $(pwd)/.env:/opt/4redteam/.env \
  jaysteelmind/4redteam /opt/4redteam/bin/ctester -config /opt/4redteam/conf/ollama-qwq32b-fp16-tc.provider.yml
```
...
LLM_SERVER_CONFIG_PATH=/opt/4redteam/conf/openrouter.provider.yml  # or deepinfra.provider.yml or deepseek.provider.yml or custom-openai.provider.yml or moonshot.provider.yml
...
LLM_SERVER_CONFIG_PATH=/opt/4redteam/conf/custom-openai.provider.yml
...
LLM_SERVER_CONFIG_PATH=/opt/4redteam/conf/moonshot.provider.yml
...
If you already have a running 4RedTeam container and want to test the current configuration:

```bash
# Run ctester in an existing container using current environment variables
docker exec -it 4redteam /opt/4redteam/bin/ctester -verbose

# Test specific agent types with deterministic ordering
docker exec -it 4redteam /opt/4redteam/bin/ctester -agents simple,primary_agent,pentester -groups basic,knowledge -verbose

# Generate a report file inside the container
docker exec -it 4redteam /opt/4redteam/bin/ctester -report /opt/4redteam/data/agent-test-report.md

# Access the report from the host
docker cp 4redteam:/opt/4redteam/data/agent-test-report.md ./
```
...
4RedTeam uses vector embeddings for semantic search, knowledge storage, and memory management. The system supports multiple embedding providers that can be configured according to your needs and preferences.

### Supported Embedding Providers

4RedTeam supports the following embedding providers:
...
EXTERNAL_SSL_CA_PATH=           # Path to custom CA certificate file (PEM format) inside the container
                                # Must point to /opt/4redteam/ssl/ directory (e.g., /opt/4redteam/ssl/ca-bundle.pem)
...
```bash
# Default location (if REDTEAM_SSL_DIR is not set)
cp ca-bundle.pem ./redteam-ssl/

# Or custom location (if using REDTEAM_SSL_DIR in docker-compose.yml)
cp ca-bundle.pem /path/to/your/ssl/dir/
```

**Step 3:** Set the path in `.env` file (path must be inside the container):
```bash
# The volume redteam-ssl is mounted to /opt/4redteam/ssl inside the container
EXTERNAL_SSL_CA_PATH=/opt/4redteam/ssl/ca-bundle.pem
EXTERNAL_SSL_INSECURE=false
```

**Step 4:** Restart 4RedTeam:
```bash
docker compose restart 4redteam
```

**Notes:**
- The `redteam-ssl` volume is mounted to `/opt/4redteam/ssl` inside the container
- You can change host directory using `REDTEAM_SSL_DIR` variable in docker-compose.yml
...
4RedTeam includes a specialized `etester` utility for testing, managing, and debugging embedding functionality. This tool is essential for diagnosing and resolving issues related to vector embeddings and knowledge storage.
...
If you're running 4RedTeam in Docker, you can use etester from within the container:

```bash
# Test embedding provider
docker exec -it 4redteam /opt/4redteam/bin/etester test

# Show detailed database information
docker exec -it 4redteam /opt/4redteam/bin/etester info -verbose
```
...
docker exec -it 4redteam /opt/4redteam/bin/etester search -query "Security vulnerability" -doc_type guide -threshold 0.8

# Filter by flow ID
docker exec -it 4redteam /opt/4redteam/bin/etester search -query "Code examples" -doc_type code -flow_id 42

# All available search options
docker exec -it 4redteam /opt/4redteam/bin/etester search -help
```
...
- **Mock Mode**: Test functions without a live 4RedTeam deployment using built-in mocks
...
The `describe` function provides detailed information about tasks and subtasks within a flow. This is particularly useful for diagnosing issues when 4RedTeam encounters problems or gets stuck.
...
When 4RedTeam gets stuck in a flow:
...
If you have 4RedTeam running in Docker, you can use ftester from within the container:

```bash
# Run ftester inside the running 4RedTeam container
docker exec -it 4redteam /opt/4redteam/bin/ftester [arguments]

# Examples:
docker exec -it 4redteam /opt/4redteam/bin/ftester -flow 123 describe
docker exec -it 4redteam /opt/4redteam/bin/ftester -flow 123 terminal -command "ps aux" -message "List processes"
```
...
```bash
docker build -t local/4redteam:latest .
```

> [!NOTE]
> You can use `docker buildx` to build the image for different platforms like a `docker buildx build --platform linux/amd64 -t local/4redteam:latest .`
>
> You need to change image name in docker-compose.yml file to `local/4redteam:latest` and run `docker compose up -d` to start the server or use `build` key option in [docker-compose.yml](docker-compose.yml) file.
...
### 4RedTeam Core License

**4RedTeam Core**: Licensed under [MIT License](LICENSE)  
Copyright (c) 2025 4RedTeam Development Team
