...
export const generateCertificates = (): void => {
    ...
    -subj "/C=US/ST=NY/L=NY/O=4RedTeam/OU=Project/CN=4RedTeam CA" \
    ...
    -subj "/C=US/ST=NY/L=NY/O=4RedTeam/OU=Project/CN=localhost" \
    -out ${SSL_PATHS.serverCsr}`,
    );

    // Create temporary configuration file
    const extFile = join(SSL_PATHS.sslDir, 'extfile.tmp');
    const extFileContent = ['subjectAltName=DNS:4redteam.local', 'keyUsage=critical,digitalSignature,keyAgreement'].join(
        '\n',
    );
